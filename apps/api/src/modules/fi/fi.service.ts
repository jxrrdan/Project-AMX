...
export class FiService {
  ...
  async addToDeal(dto: AddDealProductDto) {
    const product = await this.prisma.financeProduct.findUnique({ where: { id: dto.productId } });
    if (!product) {
  ...
  recordDisclosure(dealFinanceProductId: string, dto: RecordDisclosureDto) {
